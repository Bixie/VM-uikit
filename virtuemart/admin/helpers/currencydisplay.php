...
class CurrencyDisplay {
	...
	public function createPriceDiv($name,$description,$product_price,$priceOnly=false,$switchSequel=false,$quantity = 1.0,$forceNoLabel=false){
		...
		if(!empty($this->_priceConfig[$name][0])){
			...
			if($forceNoLabel) {
				return '<div class="uk-price Price'.$name.' '.$vis.'"><span class="Price'.$name.'" >'.$priceFormatted.'</span></div>';
			}
			$descr = '';
			if($this->_priceConfig[$name][2]) $descr = JText::_($description);
			// 			vmdebug('createPriceDiv $name '.$name.' '.$product_price[$name]);
			if(!$switchSequel){
				return '<div class="uk-price Price'.$name.' '.$vis.'" >'.$descr.'<span class="Price'.$name.'" >'.$priceFormatted.'</span></div>';
			} else {
				return '<div class="uk-price Price'.$name.' '.$vis.'" ><span class="Price'.$name.'" >'.$priceFormatted.'</span>'.$descr.'</div>';
			}
		}
